...
class OAuthClient
{
    ...
    */
     ...
     * @param string $certificate_file Use a certificate file to trust the server. Optional, defaults to null.
     * @return void
     */
    public function __construct(
        ...
        if (!extension_loaded('curl')) {
            throw new OAuthException(
                'The PHP exention curl must be installed to use this library.',
                OAuthException::CURL_NOT_FOUND
            );
        }
        ...
        if (!empty($this->certificate_file)  && !is_file($this->certificate_file)) {
            throw new OAuthInvalidArgumentException(
                'The certificate file was not found',
                OAuthInvalidArgumentException::CERTIFICATE_NOT_FOUND
            );
        }
    }
     ...
     * Array of extra parameters like scope or state (Ex: array('scope' => null, 'state' => ''))
     ...
     * @param int    $grant_type        Grant Type (@see GrantType Classes)
    ...
    {
        require_once(
            dirname(__FILE__) . DIRECTORY_SEPARATOR . 'GrantType' . DIRECTORY_SEPARATOR . 'AuthorizationCode.php'
        );

        if (!$grant_type) {
            throw new OAuthInvalidArgumentException(
                'The grant_type is mandatory.',
                OAuthInvalidArgumentException::INVALID_GRANT_TYPE
            );
        }
        ...
        if (!defined($grantTypeClass . '::GRANT_TYPE')) {
            throw new OAuthException(
                'Unknown constant GRANT_TYPE for class ' . $grantTypeClassName,
                OAuthException::GRANT_TYPE_ERROR
            );
        }
        ...
        switch ($this->client_auth) {
            ...
            case self::AUTH_TYPE_AUTHORIZATION_BASIC:
                $parameters['client_id'] = $this->client_id;
                $http_headers['Authorization'] = 'Basic ' . base64_encode(
            ...
            default:
                throw new OAuthException(
                    ...
                    OAuthException::INVALID_CLIENT_AUTHENTICATION_TYPE
                );
        }
        ...
        if ($this->access_token) {
            switch ($this->access_token_type) {
                case self::ACCESS_TOKEN_URI:
                    if (is_array($parameters)) {
                        ...
                        throw new OAuthInvalidArgumentException(
                            ...
                            OAuthInvalidArgumentException::REQUIRE_PARAMS_AS_ARRAY
                        );
                    }
                    break;
                ...
                case self::ACCESS_TOKEN_MAC:
                    $http_headers['Authorization'] = 'MAC ' . $this->generateMACSignature(
                        $protected_resource_url,
                        $parameters,
                        $http_method
                    );
                ...
                default:
                    throw new OAuthException(
                        ...
                        OAuthException::INVALID_ACCESS_TOKEN_TYPE
                    );
            }
        }
        return $this->executeRequest(
    ...
    private function executeRequest(
        ...
        if ($curl_error = curl_error($ch)) {
            throw new OAuthException($curl_error, OAuthException::CURL_ERROR);
        } else {
