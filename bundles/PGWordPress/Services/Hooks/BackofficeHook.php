...
namespace PGI\Impact\PGWordPress\Services\Hooks;
...
class BackofficeHook
{
    ...
    {
        try {
            $this->logger->debug("Building backoffice output.");

            /** @var OutputHandler $output */
            $output = $this->outputProvider->getZoneOutput('BACK.PAYGREEN');

            $this->wordpressResourceHandler->insertResources($output->getResources());

            echo $output->getContent();
        } catch (Exception $exception) {
            $text = "Error during backoffice building : " . $exception->getMessage();
